...
import Link from "next/link";
import { useState } from "react";
import { FaChartBar, FaCar, FaCarAlt, FaUser, FaSignOutAlt, FaTimes, FaBars } from "react-icons/fa";
import { useRouter } from "next/navigation";
...
const SideBar = () => {
  ...
  return (
    <div className="z-50">
      {/* Mobile Header */}
      {!sidebarOpen && (
        <div className="flex items-center justify-between p-4 bg-[#121212] text-white h-16 w-full fixed top-0 left-0 z-10 md:hidden shadow-sm">
          <h1 className="text-xl font-semibold font-[Poppins] tracking-wide">Camelcon</h1>
          <button
            onClick={() => setSidebarOpen(true)}
      ...
      <aside
        className={`${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 transform fixed top-0 left-0 w-64 md:w-64 bg-[#121212] text-white h-full transition-transform duration-300 z-30 shadow-2xl`}
      >
        <div className="flex justify-between items-center px-6 py-6 border-b border-[#1a1a1a]">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="text-white hover:text-blue-400 text-xl md:hidden transition-colors"
          >
            <FaTimes />
          </button>
        </div>

        <nav className="flex flex-col p-4 gap-2 mt-2 overflow-y-auto">
          {/* Dashboard */}
          <Link 
            href="/admin" 
            className="flex items-center gap-4 hover:bg-[#1a1a1a] p-3 rounded-lg transition-all duration-300 group"
          >
            <div className="p-2 bg-[#1a1a1a] rounded-lg group-hover:bg-[#262626] transition-colors">
          ...
          >
            ...
            <span className="font-medium tracking-wide">Cars</span>
          </Link>
          ...
          <button
            onClick={handleLogout}
            className="flex items-center gap-4 hover:bg-red-500/10 p-3 rounded-lg transition-all duration-300 group mt-6"
          >
            <div className="p-2 bg-red-500/10 rounded-lg group-hover:bg-red-500/20 transition-colors">
              <ThemedIcon icon={FaSignOutAlt} className="w-5 h-5 text-red-400 group-hover:text-red-500" />
            </div>
